...
enum ScreenshotMode {
    ...
    static func demoBooks() -> [UserBook] {
        ...
        // Add reading sessions to the stats book
        statsBook.readingSessions = [
            ReadingSession(date: Date(timeIntervalSinceNow: -86400 * 3), durationMinutes: 45, pagesRead: 20),
            ReadingSession(date: Date(timeIntervalSinceNow: -86400 * 2), durationMinutes: 30, pagesRead: 15),
            ReadingSession(date: Date(timeIntervalSinceNow: -86400 * 1), durationMinutes: 60, pagesRead: 25)
        ]
        
        return [heroBook, searchBook, culturalBook, themeBook, statsBook]
    }
}